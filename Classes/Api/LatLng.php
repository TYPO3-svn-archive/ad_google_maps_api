 ...
 * @api
 */
class Tx_AdGoogleMapsApi_Api_LatLng {
	...
	public function setLatLng($latitude, $longitude = NULL) {
		if ($longitude === NULL) {
			if (self::isValidCoordinate($latitude) === FALSE) {
				throw new Tx_AdGoogleMapsApi_Exception('Invalid property value for Tx_AdGoogleMapsApi_Api_LatLng::setLatLng ("' . $latitude . '") given.', 1294069148);
			}
			...
			throw new Tx_AdGoogleMapsApi_Exception('Invalid property value for Tx_AdGoogleMapsApi_Api_LatLng::setLatLng ("' . $latitude . ', ' . $longitude . '") given.', 1294069149);
		}
		$this->latitude = (float) $latitude;
		$this->longitude = (float) $longitude;
	}
