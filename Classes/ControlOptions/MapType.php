 ...
 * @api
 */
class Tx_AdGoogleMapsApi_ControlOptions_MapType extends Tx_AdGoogleMapsApi_ControlOptions_AbstractControlOptions {
	...
	public function __construct($mapTypeIds = NULL, $position = NULL, $style = NULL) {
		$this->setMapTypeIds($mapTypeIds ? $mapTypeIds : self::STYLE_DEFAULT);
		$this->setPosition($position ? $position : self::POSITION_TOP_RIGHT);
	...
	/**
	 * Returns mapTypeControl options as JavaScript string.
	 *
	 * @return string
	 */
	public function getPrintOptions() {
		$options = array();
		if (count($this->mapTypeIds)) {
			$options[] = 'mapTypeIds: [ ' . implode(', ', $this->mapTypeIds) . ' ]';
		}
		if ($this->position !== self::POSITION_TOP_RIGHT) {
	...
	public function getPrint() {
		return 'mapTypeControlOptions: { ' . $this->getPrintOptions() . ' }';
	}
