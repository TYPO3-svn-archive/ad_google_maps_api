 ...
 * @api
 */
class Tx_AdGoogleMapsApi_LatLngArray {

	/**
	 * @var SplObjectStorage<Tx_AdGoogleMapsApi_LatLng>
	 */
	 ...
	 * Constructor.
	 * $latLngArray can be 
	 * - a SplObjectStorage of Tx_AdGoogleMapsApi_LatLng objects, 
	...
	public function setLatLngArray($latLngArray) {
		if ($latLngArray instanceof SplObjectStorage) {
			...
			throw new Tx_AdGoogleMapsApi_Exception('Invalid property value for Tx_AdGoogleMapsApi_LatLngArray::setLatLngArray().', 1294069158);
		}
	}
	 ...
	 *
	 * @return SplObjectStorage<Tx_AdGoogleMapsApi_LatLng>
	 */
	...
	public function addLatLng($latLng) {
		return $this->latLngArray->attach(new Tx_AdGoogleMapsApi_LatLng($latLng));
	}
	 ...
	 * @return void
	 */
	public function removeLatLng(Tx_AdGoogleMapsApi_LatLng $latLng) {
	...
	public function getPrint() {
		...
		return 'new google.maps.MVCArray(' . (count($latLngs) ? '[ ' . implode(', ', $latLngs) . ' ]' : '') . ')';
	}
